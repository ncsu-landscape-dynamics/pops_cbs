...
prior_means <- read.csv(paste0(cbs_out, "posterior_means_pa_2013.csv"))
prior_means <- prior_means[[1]]
prior_cov_matrix <- read.csv(paste0(cbs_out, "posterior_cov_matrix_pa_2013.csv"))

start_time <- Sys.time()

# Calibration for PoPS model
cal_2014 <- PoPS::calibrate(
  infected_years_file = paste0(cbs_path, "infection/cbs_pa_2015.tif"),
  number_of_observations = 33,
  prior_number_of_observations = 190,
  prior_means,
  prior_cov_matrix,
  ...
  infected_file_list = paste0(cbs_path, "infection/cbs_pa_2014.tif"),
...
file_name <- paste0(cbs_out, "posterior_means_pa_2014.csv")
write.csv(cal_2014$posterior_means, file_name, row.names = FALSE)

file_name <- paste0(cbs_out, "posterior_cov_matrix_pa_2014.csv")
