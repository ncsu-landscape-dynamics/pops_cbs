...
prior_means <- read.csv(paste0(cbs_out, "posterior_means_2014.csv"))
prior_means <- prior_means$x
prior_means <- prior_means[1:6]

# posterior covariance matrix from previous calibration
prior_cov_matrix <- read.csv(paste0(cbs_out, "posterior_cov_matrix_2014.csv"))
prior_cov_matrix <- as.matrix(prior_cov_matrix)

# Calibration for PoPS model
cal_2015 <- PoPS::calibrate(
  infected_years_file = paste0(cbs_path, "infection/cbs_2016.tif"),
  number_of_observations = 33,
  prior_number_of_observations = 222,
  ...
  infected_file_list = paste0(cbs_path, "infection/cbs_2015.tif"),
  host_file_list = paste0(cbs_path, "host/host.tif"),
  total_populations_file = paste0(cbs_path, "total_pops_file.tif"),
  temp = TRUE,
  temperature_coefficient_file = paste0(cbs_path, "temp/temp_coeff_2015_.tif"),
  precip = TRUE,
...
file_name <- paste0(cbs_out, "posterior_means_2015.csv")
write.csv(cal_2015$posterior_means, file_name, row.names = FALSE)

file_name <- paste0(cbs_out, "posterior_cov_matrix_2015.csv")
write.csv(cal_2015$posterior_cov_matrix, file_name, row.names = FALSE)