...
prior_means <- read.csv(paste0(cbs_out, "posterior_means_pa_2014.csv"))
prior_means <- prior_means[[1]]
prior_cov_matrix <- read.csv(paste0(cbs_out, "posterior_cov_matrix_pa_2014.csv"))

start_time <- Sys.time()

# Calibration for PoPS model
cal_2015 <- PoPS::calibrate(
  infected_years_file = paste0(cbs_path, "infection/cbs_pa_2016.tif"),
  ...
  infected_file_list = paste0(cbs_path, "infection/cbs_pa_2015.tif"),
...
file_name <- paste0(cbs_out, "posterior_means_pa_2015.csv")
write.csv(cal_2015$posterior_means, file_name, row.names = FALSE)

file_name <- paste0(cbs_out, "posterior_cov_matrix_pa_2015.csv")
