...
prior_means <- read.csv(paste0(cbs_out, "posterior_means_pa_2015.csv"))
prior_means <- prior_means[[1]]
prior_cov_matrix <- read.csv(paste0(cbs_out, "posterior_cov_matrix_pa_2015.csv"))

start_time <- Sys.time()

# Calibration for PoPS model
cal_2016 <- PoPS::calibrate(
  infected_years_file = paste0(cbs_path, "infection/cbs_pa_2017.tif"),
  number_of_observations = 9,
  prior_number_of_observations = 248,
  ...
  infected_file_list = paste0(cbs_path, "infection/cbs_pa_2016.tif"),
  host_file_list = paste0(cbs_path, "host/host.tif"),
  total_populations_file = paste0(cbs_path, "total_pops_file.tif"),
  temp = TRUE,
  temperature_coefficient_file = paste0(cbs_path, "temp/temp_coeff_2017.tif"),
  precip = TRUE,
  precipitation_coefficient_file = paste0(cbs_path, "precip/prcp_coeff_2017_.tif"),
  model_type = "SI",
  latency_period = 0,
  time_step = 'day',
...
file_name <- paste0(cbs_out, "posterior_means_pa_2016.csv")
write.csv(cal_2016$posterior_means, file_name, row.names = FALSE)

file_name <- paste0(cbs_out, "posterior_cov_matrix_pa_2016.csv")
