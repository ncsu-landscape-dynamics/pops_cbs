...
prior_means <- read.csv(paste0(cbs_out, "posterior_means_pa_2016.csv"))
prior_means <- prior_means[[1]]
prior_cov_matrix <- read.csv(paste0(cbs_out, "posterior_cov_matrix_pa_2016.csv"))

start_time <- Sys.time()

# Calibration for PoPS model
cal_2017 <- PoPS::calibrate(
  infected_years_file = paste0(cbs_path, "infection/cbs_pa_2018.tif"),
  ...
  infected_file_list = paste0(cbs_path, "infection/cbs_pa_2017.tif"),
...
file_name <- paste0(cbs_out, "posterior_means_pa_2017.csv")
write.csv(cal_2017$posterior_means, file_name, row.names = FALSE)

file_name <- paste0(cbs_out, "posterior_cov_matrix_pa_2017.csv")
