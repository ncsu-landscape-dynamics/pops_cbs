...
prior_means <- read.csv(paste0(cbs_out, "posterior_means_pa_2017.csv"))
prior_means <- prior_means[[1]]
prior_cov_matrix <- read.csv(paste0(cbs_out, "posterior_cov_matrix_pa_2017.csv"))

start_time <- Sys.time()

# Calibration for PoPS model
cal_2018 <- PoPS::calibrate(
  infected_years_file = paste0(cbs_path, "infection/cbs_pa_2019.tif"),
  number_of_observations = 36,
  prior_number_of_observations = 270,
  ...
  infected_file_list = paste0(cbs_path, "infection/cbs_pa_2018.tif"),
  host_file_list = paste0(cbs_path, "host/host.tif"),
  total_populations_file = paste0(cbs_path, "total_pops_file.tif"),
  temp = TRUE,
  temperature_coefficient_file = paste0(cbs_path, "temp/temp_coeff_2019.tif"),
  precip = TRUE,
  precipitation_coefficient_file = paste0(cbs_path, "precip/prcp_coeff_2019.tif"),
  model_type = "SI",
  latency_period = 0,
  time_step = 'day',
...
file_name <- paste0(cbs_out, "posterior_means_pa_2018.csv")
write.csv(cal_2018$posterior_means, file_name, row.names = FALSE)

file_name <- paste0(cbs_out, "posterior_cov_matrix_pa_2018.csv")
