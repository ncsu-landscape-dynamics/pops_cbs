...
for (yr in seq(12, 21)) {
  parameter_means <- get(paste0("cal11_", yr))$posterior_means
  parameter_cov_matrix <- get(paste0("cal11_", yr))$posterior_cov_matrix
  
  print(paste0("Working on ", yr))
  
  val_cbs <- validate(
    infected_years_file = paste0(cbs_path, "infection/cbs_20", yr+1, ".tif"),
    number_of_iterations = 100,
    number_of_cores = 7,
    parameter_means,
    ...
    pest_host_table = paste0(cbs_path, "pest_host_table_cbs.csv"),
    competency_table = paste0(cbs_path, "competency_table_cbs.csv"),
    infected_file_list = paste0(cbs_path, "infection/cbs_20", yr, ".tif"),
    host_file_list = paste0(cbs_path, "host/host.tif"),
    total_populations_file = paste0(cbs_path, "total_pops_file.tif"),
    temp = TRUE,
    temperature_coefficient_file = paste0(cbs_path, "temp/temp_coeff_20", yr+1, ".tif"),
    precip = TRUE,
    precipitation_coefficient_file = paste0(cbs_path, "precip/prcp_coeff_20", yr+1, ".tif"),
    model_type = "SI",
    latency_period = 0,
    time_step = "day",
    season_month_start = 4,
  ...
  file_name <- paste0(cbs_out, "validation_outputs_20", yr+1, ".rdata")
  save(val_cbs, file = file_name)
  
  print(paste0("Finished with ", yr))
}
